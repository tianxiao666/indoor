...
public class LeftSliderLayout extends ViewGroup {
	...
	/**
	 * Values for VelocityTracker to compute current velocity. VELOCITY_UNITS in
	 * dp mVelocityUnits in px
	 */
	private static final int VELOCITY_UNITS = 1000;
	private int mVelocityUnits;

	/**
	 * The minimum velocity for determining the direction. MINOR_VELOCITY in dp
	...
	/**
	 * The width of Sliding distance from left. And it should be the same with
	 * the width of the View below SliderLayout in a FrameLayout. DOCK_WIDTH in
	 * dp mDockWidth in px
	 */
	...
	/**
	 * The default values of shadow. VELOCITY_UNITS in dp mVelocityUnits in px
	 */
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode != MeasureSpec.EXACTLY) {
			throw new IllegalStateException(
					"LeftSliderLayout only can run at EXACTLY mode!");
		}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mMainChild != null) {
			mMainChild.layout(l, t, l + mMainChild.getMeasuredWidth(), t
					+ mMainChild.getMeasuredHeight());
		}
		...
		if (nCount > 1) {
			...
			leftChild.layout(l - nLeftChildWidth, t, l,
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (mMainChild != null && mTouchState != TOUCH_STATE_SCROLLING
			...
			if (!rect.contains((int) event.getX() + nCurScrollX,
					(int) event.getY())) {
				return false;
			}
		}
		...
		case MotionEvent.ACTION_UP: {
			...
			// Set open or close state, when get ACTION_UP or ACTION_CANCEL
			// event.
			if (nCurScrollX < 0) {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mListener != null
				&& !mListener.OnLeftSliderLayoutInterceptTouch(ev)) {
			return false;
		}
	 ...
	 */
	public void setOnLeftSliderLayoutListener(
			OnLeftSliderLayoutStateListener listener) {
		mListener = listener;
	}
	...
	public interface OnLeftSliderLayoutStateListener {
		 ...
		 * @param ev
		 *            Touch Event
		 * @return true - LeftSliderLayout need to manage the
		 *         InterceptTouchEvent. false - LeftSliderLayout don't need to
		 *         manage the InterceptTouchEvent.
		 */
