...
class CDAOCB_FLOOR extends CCDAOCB_FLOOR {
	...
	function getNextSeqId() {
		return ($this->DB ()->nextId ( $this->_cb_floor_sequence ));
	}
	// 获取分页信息
	...
	function getFloor_list($pageSize, $pageNum, $whereSql, $BUILDING_ID) {
		$sql = "select * from {$this->_table} WHERE "; // FLOOR_NAME= '{$FLOOR_NAME}' and BUILDING_ID= '{$BUILDING_ID}'";
	...
	function getFloor_id($building_id) {
		$sql = "SELECT * FROM {$this->_table} where BUILDING_ID={$building_id}";
		$result = $this->DB ()->getAll ( $sql );
		if ($result) {
			return $result;
		} else {
	 ...
	 * 通过场所ID获取一个楼层ID
	 */
	function getFloorIdByBuildId($BUILDING_ID) {
		$sql = "SELECT FLOOR_ID FROM {$this->_table}" . " WHERE BUILDING_ID={$BUILDING_ID} ORDER BY PHYSICAL_FLOOR";
		$result = $this->DB ()->getAll ( $sql );
		if (! empty ( $result )) {
	 ...
	 */
	function getFloorListByWhere($where) {
