				...
				if (empty ( $message )) {
					if ($jsonArrayObj->LONGITUDE != "" && $jsonArrayObj->LATITUDE != "") {//判断是否有当前坐标
						...
						if (! empty ( $building_list )) {
							...
							$location = (new Location ())->getLocation ( $jsonArrayObj->APLIST, $building_list [0] ['BUILDING_ID'], $jsonArrayObj->FLOOR_ID, $jsonArrayObj->DRAW_MAP_ID );
							if ($location == null) {
								$location = array ();
							}
							$maxAp = null;
							...
							//生成AP查询条件
							if($maxAp->MAC_BSSID) {
								$maxAp->MAC_BSSID = str_replace ( "-", ":", $maxAp->MAC_BSSID );
							}
							$apSql = " MAC_BSSID='" . strtoupper ( $maxAp->MAC_BSSID ) . "' and BUILDING_ID='" . $building_list [0] ['BUILDING_ID'] . "'";
							if ($maxAp->FREQUENCY) {
							...
							$content = $this->arrayToJson ( array (
									...
									"LayerList" => json_encode ( $floorSvgInfo['LayerList'] ) ,
