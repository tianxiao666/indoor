...
class SvgUtil {
	...
	public static function getSvgInfo($SvgSrc) {
		...
		return ((new XMLUtil ())->getXMLParseStructInfo ( $XMLParseStruct_Value, 0 ));
	}
	/**
	 * 获取SVG图层指定Tag的Value
	 *
	 ...
	 * 获取真实的属性值
	 */
	public static function getSvgElementAttrValue($tag, $attrname, $attrvalue) {
		$NotEssential = array (
				"STROKE" => array (
						"null" => false 
				),
		...
		$NotEssentialAttrValue = $NotEssential [strtoupper ( $attrname )];
		if ($NotEssentialAttrValue != null) {
			$realattrvalue = $NotEssentialAttrValue [strtolower ( $attrvalue )];
			if ($realattrvalue !== null) {
				if ($realattrvalue === false) {
					$realattrvalue = null;
				}
			...
			if (! empty ( $DRAW_MAP_ID )) {
				...
				if (! empty ( $LayerFormList )) {
					foreach ( $LayerFormList as $LayerForm ) {
						...
						$layersrc = $dom->saveXML ( $g );
						if (! empty ( $layersrc )) {
							$LayerList [$LayerForm ["LAYER_TYPE"]] = "1";
						}
					}
				}
				$SVGSRC = $dom->saveXML ();
