...
KISSY.add('suggest', function(S, undefined) {
     ...
     * @param {Object} config
     */
    function Suggest(textInput, selid ,dataSource, config) {
        var self = this, cbFn;
        selId = selid;
...
//	             // 只有一个选项时，选中该项
	             if (items.length == 1){
	                 if (!self.selectedItem) {
		                 ...
		                 if (newSelectedItem) {
		                     ...
		                     self.textInput.wid   = self._getSelectedItemVal(self.selectedItem);
		                     document.getElementById(self.selId).value= self.textInput.wid;
		                 }
	                 } 
            	 }
                self.stop();
        ...
        _handleResponse: function(data) {
            ...
            if ((len = formattedData.length) > 0) {
                ...
                for (i = 0; i < len; ++i) {
                    ...
                    DOM.attr(li, KEY, key);
                    DOM.attr(li, RESULT, itemData[RESULT]);
        ...
        _getSelectedItemVal: function() {
            ...
            return DOM.attr(self.selectedItem, RESULT);
        },
