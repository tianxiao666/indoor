...
public class SVGParser {
	...
	private static Matrix parseTransform(String s) {
		if (s.startsWith("matrix(")) {
			...
			if (np.numbers.size() > 0) {
				float tx = getZoomValue(np.numbers.get(0));
				float ty = 0;
				if (np.numbers.size() > 1) {
					ty = getZoomValue(np.numbers.get(1));
				}
				Matrix matrix = new Matrix();
			...
			if (np.numbers.size() > 0) {
				...
				if (np.numbers.size() > 2) {
					cx = getZoomValue(np.numbers.get(1));
					cy = getZoomValue(np.numbers.get(2));
				}
				Matrix matrix = new Matrix();
	...
	private static Path doPath(String s) {
		...
		while (ph.pos < n) {
			...
			case 'M':
			case 'm': {
				float x = getZoomValue(ph.nextFloat());
				float y = getZoomValue(ph.nextFloat());
				if (cmd == 'm') {
					// subPathStartX += x;
					// subPathStartY += y;
					p.rMoveTo(x, y);
			...
			case 'c': {
				wasCurve = true;
				float x1 = getZoomValue(ph.nextFloat());
				float y1 = getZoomValue(ph.nextFloat());
				float x2 = getZoomValue(ph.nextFloat());
				float y2 = getZoomValue(ph.nextFloat());
				float x = getZoomValue(ph.nextFloat());
				float y = getZoomValue(ph.nextFloat());
				if (cmd == 'c') {
					x1 += lastX;
			...
			case 'A':
			case 'a': {
				float rx = getZoomValue(ph.nextFloat());
				float ry = getZoomValue(ph.nextFloat());
				float theta = ph.nextFloat();
				float largeArc = getZoomValue(ph.nextFloat());
				float sweepArc = getZoomValue(ph.nextFloat());
				float x = getZoomValue(ph.nextFloat());
				float y = getZoomValue(ph.nextFloat());
	...
	public static void setZoomValue(float z) {
		zoom = z;
	}

	private static Float getZoomValue(Float f) {
		if (f != null) {
			return (zoom * f);
		}
		return (null);
	...
	private static class SVGHandler extends DefaultHandler {
		...
		private boolean doStroke(Properties atts) {
			...
			if (color != null) {
				...
				Float width = getZoomValue(atts.getFloat("stroke-width"));
				// Set defaults
		...
		private Gradient doGradient(boolean isLinear, Attributes atts) {
			...
			if (isLinear) {
				gradient.x1 = getZoomValue(getFloatAttr("x1", atts, 0f));
				gradient.x2 = getZoomValue(getFloatAttr("x2", atts, 0f));
				gradient.y1 = getZoomValue(getFloatAttr("y1", atts, 0f));
				gradient.y2 = getZoomValue(getFloatAttr("y2", atts, 0f));
			} else {
				gradient.x = getZoomValue(getFloatAttr("cx", atts, 0f));
				gradient.y = getZoomValue(getFloatAttr("cy", atts, 0f));
				gradient.radius = getZoomValue(getFloatAttr("r", atts, 0f));
			}
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (boundsMode) {
				if (localName.equals("rect")) {
					Float x = getZoomValue(getFloatAttr("x", atts));
					if (x == null) {
						x = 0f;
					}
					Float y = getZoomValue(getFloatAttr("y", atts));
					if (y == null) {
						y = 0f;
					}
					Float width = getZoomValue(getFloatAttr("width", atts));
					Float height = getZoomValue(getFloatAttr("height", atts));
					bounds = new RectF(x, y, x + width, y + height);
				}
				return;
			}
			if (localName.equals("svg")) {
				int width = (int) Math.ceil(getZoomValue(getFloatAttr("width",
						atts)));
				int height = (int) Math.ceil(getZoomValue(getFloatAttr(
						"height", atts)));
				...
				if (gradient != null) {
					float offset = getZoomValue(getFloatAttr("offset", atts));
				...
				Float x1 = getZoomValue(getFloatAttr("x1", atts));
				Float x2 = getZoomValue(getFloatAttr("x2", atts));
				Float y1 = getZoomValue(getFloatAttr("y1", atts));
				Float y2 = getZoomValue(getFloatAttr("y2", atts));
				Properties props = new Properties(atts);
				if (doStroke(props)) {
					pushTransform(atts);
					doLimits(x1, y1);
				...
				Float centerX = getZoomValue(getFloatAttr("cx", atts));
				Float centerY = getZoomValue(getFloatAttr("cy", atts));
				Float radius = getZoomValue(getFloatAttr("r", atts));
				...
				Float centerX = getZoomValue(getFloatAttr("cx", atts));
				Float centerY = getZoomValue(getFloatAttr("cy", atts));
				Float radiusX = getZoomValue(getFloatAttr("rx", atts));
				Float radiusY = getZoomValue(getFloatAttr("ry", atts));
				...
				if (numbers != null) {
					...
					if (points.size() > 1) {
						pushTransform(atts);
						Properties props = new Properties(atts);
						p.moveTo(getZoomValue(points.get(0)),
								getZoomValue(points.get(1)));
						for (int i = 2; i < points.size(); i += 2) {
							float x = getZoomValue(points.get(i));
							float y = getZoomValue(points.get(i + 1));
							p.lineTo(x, y);
						}
				...
				text_s = getFloatAttr("font-size", atts);
				text_t = "";
			} else if (!hidden) {
		...
		public void characters(char ch[], int start, int length) {
			if (text_t != null) {
				text_t = text_t + String.valueOf(ch, start, length);
			}
		}
			...
			if (localName.equals("svg")) {
				...
				if (localName.equals("text")) {
					if ((text_x != null) && (text_y != null)
							&& (text_t != null) && (text_t.length() > 0)) {
						if (text_s != null) {
							paint.setTextSize(text_s);
						}
						canvas.drawText(text_t, text_x, text_y, paint);
					}
					text_x = null;
					text_y = null;
					text_s = null;
