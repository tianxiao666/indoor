...
class BuildingFloorPlanegraphMgrController extends AdminController {
	...
	public function actionIndex() {
		...
		if (! empty ( $planegraph_list )) {
			...
			$where_thm = "";
			if (! empty ( $where_ids )) {
				$where_thm = "(" . $where_ids . ") AND PIC_TYPE='{$CDAO_Pic->FILE_THM}'";
			}
			//svg源码文件路径搜索条件
			$where_svg ="(" . $where_ids . ") AND PIC_TYPE='{$CDAO_Pic->FILE_SVG}'";
			//svg原图路径list
			$svgPathList = $CDAO_Pic->getSvgPicPathListByWhere ( $where_svg );
			$ThumbnailPathList = $CDAO_Pic->getSvgPicPathListByWhere ( $where_thm );
			$pageData ["ThumbnailSize"] = $CDAO_Pic->FILE_THM_SIZE;
		}
		//画布比例，用来计算查看时平面图html的大小
		$pageData ["SVG_DW_SCALE"] = (new CDict ())->DEFAULT_PLANEGRAPH ["DW_SCALE"];
		//svg原图路径list
		$pageData ["svgPathList"] = $svgPathList;
		...
		$pageData ["DW_SCALE"] = "1px:" . ((new CDict ())->DEFAULT_PLANEGRAPH ["DW_SCALE"] * 100) . "cm";
		$p = new page ( $total );
