...
ImgCropper.prototype = {
  ...
  GetSize: function(nowWidth, nowHeight, fixWidth, fixHeight) {
	var iWidth = nowWidth, iHeight = nowHeight, scale = iWidth / iHeight;hscale = iHeight/iWidth;
	//alert(scale);
	//按比例设置
	if (fixWidth==fixHeight){
		if (scale>hscale && fixHeight){ iWidth = (iHeight = fixHeight) * scale;}
		if (scale<hscale && fixHeight){ iHeight = (iWidth = fixWidth) * hscale;}
	}else{
