...
class CDAODM_DRAW_MAP extends CCDAODM_DRAW_MAP {
	 ...
	 * @return unknown
	 */
	function sel_draw_map($whereSql) {
		$sql = "SELECT DM_TOPIC,DRAW_MAP_ID FROM {$this->_table}";
		if (! empty ( $whereSql )) {
			$sql = $sql . " WHERE " . $whereSql;
		}
	...
	function getSeqIdByFloorIdAndStatus($FLOOR_ID, $STATUS) {
		$sql = "SELECT {$this->_table_seq_name} FROM {$this->_table}" . " WHERE FLOOR_ID={$FLOOR_ID} AND STATUS='{$STATUS}' ORDER BY MOD_TIME DESC";
		$result = $this->DB ()->getAll ( $sql );
		if (! empty ( $result )) {
			return ($result [0] [$this->_table_seq_name]);
		}
	...
	/**
	 * $key为所需要查的字段，$where为查询条件
	 *
	 * @return number
	 */
	public function getByWhere($key, $where) {
		$sql = "SELECT {$key} FROM {$this->_table}" . " WHERE " . $where;
		$result = $this->DB ()->getAll ( $sql );
		if (! empty ( $result )) {
			return $result;
		}
