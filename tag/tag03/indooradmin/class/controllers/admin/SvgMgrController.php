...
class SvgMgrController extends AdminController {
	...
	public function actionIndex() {
		...
		if (! empty ( $DRAW_MAP_ID )) {
			...
			$result = $Cdao_Poi->getAllWhere ( "POI_ID,POI_NAME,POI_TYPE,POI_NOTE,NOTE,STATUS,ADDRESS,PHONE,SVG_ID", "DRAW_MAP_ID={$DRAW_MAP_ID}" );
			if (! empty ( $result )) {
				$PoiFormList = array ();
				$PoiSvgList = array ();
				$where = "";
		...
		$pageData ["LAYERTYPE_AP"] = SvgUtil::APTYPECODE;
		$pageData ["LAYERTYPE_POI"] = SvgUtil::POITYPECODE;
					...
					if (! empty ( $ElementInfos )) {
						foreach ( $ElementInfos as $ElementInfo ) {
							if (($ElementInfo ["tag"] != "TITLE") && ($ElementInfo ["tag"] != "DESC")) {
								...
								if (($LayerForm ["LAYER_TYPE"] == SvgUtil::APTYPECODE) && ! empty ( $APFORMLIST )) {
									$ApForm = $APFORMLIST [$ElementForm ["SVG_ID"]];
									if (! empty ( $ApForm )) {
										$ApForm = get_object_vars ( $ApForm );
										$ElementForm ["POI_TYPE"] = SvgUtil::APTYPECODE;
								...
								if (($LayerForm ["LAYER_TYPE"] == SvgUtil::POITYPECODE) && ! empty ( $POIFORMLIST )) {
