...
// 					newText.textContent = "text";
	...
	var mouseMove = function(evt)
	{
		...
		var selected = selectedElements[0],
			pt = transformPointForFireFox( evt.clientX, evt.clientY, root_sctm ),
			mouse_x = pt.x * current_zoom,
			mouse_y = pt.y * current_zoom,
			shape = getElem(getId());
...
// Parameters:
// name - The given name
this.createLayer = function(name,type) {
 ...
 * AP图层为特定图形
 */
this.selApPoi = function(name,layerType) {
	if(layerType==""){
		 layerType=this.getLayerTypeByName(name);
	}
	if(layerType=="AP"){
...
this.clear = function() {
	...
	canvas.createLayer("图层 1",defaultLayerType);
