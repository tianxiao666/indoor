...
public class IndoorMapActivity extends wifiDetecterActivity implements
	...
	private void onHandleIntent(Intent intent) {
		...
		location_x = intent.getStringExtra("X");
		location_y = intent.getStringExtra("Y");
		if (location_x == "") {
			location_x = null;
			logError("定位坐标X为空！");
		}
				...
				if (msg.arg2 == 1) {
					...
					try {
						if ((location_x != null) && (location_y != null)) {
							fl_PlaneGraph.setLocation(
									Integer.parseInt(location_x),
									Integer.parseInt(location_y));
						} else {
							Toast.makeText(IndoorMapActivity.this, "定位失败！",
									Toast.LENGTH_SHORT).show();
							logError("设置定位坐标失败（" + location_x + ","
									+ location_y + "）");
						}
					} catch (Exception e) {
						logError("设置定位坐标失败（" + location_x + "," + location_y
								+ "）：" + e.getMessage());
					}
				} else {
					ErrorMessage = (String) msg.obj;
	...
	private void handleContent(int actionID, String content) throws Exception {
		...
		case ServiceClientInterface.ID_ACTION_GetLocation: {
			...
			location_x = contentMap.get("X");
			location_y = contentMap.get("Y");
			if (location_x == "") {
				location_x = null;
			}
			if (location_y == "") {
			...
			if (svgsrc != null) {
				...
				try {
					if ((location_x != null) && (location_y != null)) {
						fl_PlaneGraph.setLocation(Integer.parseInt(location_x),
								Integer.parseInt(location_y));
					} else {
						Toast.makeText(IndoorMapActivity.this, "定位失败！",
								Toast.LENGTH_SHORT).show();
