...
// 					}
			...
			(function() {
				...
				var dbox = function(type, msg, callback, defText) {
					$('#dialog_content').html('<p>'+msg.replace(/\n/g,'</p><p>')+'</p>')
						.toggleClass('prompt',(type=='prompt' || type=='newlayer'));
					btn_holder.empty();
					...
					if(type == 'newlayer') {
						...
						var input = $('<input type="text" maxlength="6" style="width:150px;display:inline;margin-top:1px;margin-right:35px;"></input>').appendTo(layerinfo);
						input.val(defText.layername || '');
						input.bind('keydown', 'return', function() {haskeydown = true;});
						input.bind('keyup', 'return', function() {if(haskeydown){ok.click();}});
						var typeselector = $('<select style="width:85px;">'+env.typeOption+'</select>').appendTo(layerinfo);
						typeselector.val(defText.layertype);
						typeselector.bind('keydown', 'return', function() {haskeydown = true;});
						typeselector.bind('keyup', 'return', function() {if(haskeydown){ok.click();}});
					}
					...
					ok.click(function() {
						...
						if(type=="newlayer") {
							...
							resp = {layername:layername,layertype:typeselector.val()};
							if(layername != svgedit.utilities.toXml(layername)) {
								resp.error = "名称不合法！";
							}
						} else {
							resp = (type == 'prompt')?input.val():true;
						}
						box.hide();
						if(callback) {
							callback(resp);
					...
					if(type == 'prompt' || type=='newlayer') input.focus();
				}
				...
				$.newlayer = function(msg, txt, cb) { dbox('newlayer', msg, cb, txt);};
			}());
			...
			var exportHandler = function(window, data) {
				...
				canvg(c, data.svg, {renderCallback: function() {
					...
					if(embed)
					{
						var win = (!!env.include) ? window : window.parent;
			...
			var setupFlyouts = function(holders) {
				$.each(holders, function(hold_sel, btn_opts) {
					...
					shower.mousedown(function(evt) {
						...
						var holder = $(hold_sel);
						var l = pos.left + $(show_sel).parent().width() + $(show_sel).parent().position().left * 2 + 2;
						var parentpos = $(show_sel).parent().position();
						holder.css({
							'left': l,
							'top': parentpos.top + pos.top
						});
			...
			*/
			...
			var clickClear = function(){
				...
				$.confirm(uiStrings.notification.QwantToClear, function(ok) {
					...
					$.newlayer(uiStrings.notification.enterUniqueLayerName,{layername:oldName,layertype:""}, function(jsonobj) {
						if(!!jsonobj.error) {
							$.alert(jsonobj.error);
							return;
						}
						if (!jsonobj.layername) return;
						svgCanvas.renametypeLayerByName(oldName,jsonobj.layername,jsonobj.layertype);
						updateContextPanel();
						populateLayers();
					});
				});
			...
			var clickExport = function() {
				...
				if(window.canvg) {
					...
					$.getScript(env.svgeditdir + 'canvg/rgbcolor.js', function() {
						$.getScript(env.svgeditdir + 'canvg/canvg.js', function() {
							svgCanvas.rasterExport();
						});
					});
				}
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
			...
			setTimeout(function() {
				svgCanvas.embedImage(env.svgeditdir + 'images/logo.png', function(datauri) {
					if(!datauri) {
						// Disable option
			...
			$('#layer_new').click(function() {
				...
				$.newlayer(uiStrings.notification.enterUniqueLayerName,{layername:uniqName,layertype:""}, function(jsonobj) {
					if(!!jsonobj.error) {
						$.alert(jsonobj.error);
						return;
					}
					if (!jsonobj.layername) return;
					if (svgCanvas.getCurrentDrawing().hasLayer(jsonobj.layername)) {
						$.alert(uiStrings.notification.dupeLayerName);
						return;
					}
					svgCanvas.createLayer(jsonobj.layername);
					svgCanvas.retypeLayerByName(jsonobj.layername,jsonobj.layertype);
					updateContextPanel();
					populateLayers();
				});
			});
			...
			$('#layer_rename').click(function() {
				var oldName = svgCanvas.getCurrentDrawing().getCurrentLayerName();
				var oldType = svgCanvas.getLayerTypeByName(oldName);
				$.newlayer(uiStrings.notification.enterNewLayerName,{layername:oldName,layertype:oldType}, function(jsonobj) {
					if(!!jsonobj.error) {
						$.alert(jsonobj.error);
						return;
					}
					if (!jsonobj.layername) return;
					if ((oldName != jsonobj.layername) && svgCanvas.getCurrentDrawing().hasLayer(jsonobj.layername)) {
						$.alert(uiStrings.notification.layerHasThatName);
						return;
					}
					if((oldName != jsonobj.layername) || (oldType != jsonobj.layertype)) {
						svgCanvas.renametypeLayerByName(oldName,jsonobj.layername,jsonobj.layertype);
						updateContextPanel();
						populateLayers();
					}
			...
			var populateLayers = function(){
				...
				selLayerNames.empty();
				var currentLayerName = svgCanvas.getCurrentDrawing().getCurrentLayerName();
				var layernumbers = svgCanvas.getCurrentDrawing().getNumLayers();
				...
				while (layer--) {
					...
					var layerinvis = svgCanvas.getCurrentDrawing().getLayerVisibility(name) ? "" : " layerinvis";
					var layerdeletetd = (layernumbers == 1) ? "<td style=\"width:14px\"></td>" : "<td class=\"layerdelete\" style=\"cursor:pointer;\"></td>";
					var layerrenametd = "<td class=\"layerrename\" style=\"cursor:pointer;\"></td>";
					appendstr += "<td class=\"layername\" >" + name + "</td>" + "<td class=\"layertype\" style=\"width:50px\">" + typehtml + "</td><td class=\"layervis" + layerinvis +"\"/>" + layerrenametd + layerdeletetd + "</tr>";
				...
				if(icon !== undefined) {
					...
					$.resizeSvgIcons({'td.layerrename .svg_icon':14});
				}
				var icon = $.getSvgIcon('delete');
				if(icon !== undefined) {
					$('td.layerdelete',layerlist).append(icon.clone());
					$.resizeSvgIcons({'td.layerdelete .svg_icon':14});
				}
				// handle selection of layer
				var style_mouseover = {"font-style": "italic", "color":"blue", "width":"85px"};
				var style_mouseout = {"font-style": "normal", "color":"black", "width":"85px"};
				...
				.mouseover(function(evt){
					var layername = $(this.parentNode).children("td.layername");
				...
				$('#layerlist td.layerrename').click(function(evt){
					var row = $(this.parentNode);
					var oldName = row.children("td.layername").text();
					var oldType = svgCanvas.getLayerTypeByName(oldName);
					$.newlayer(uiStrings.notification.enterNewLayerName,{layername:oldName,layertype:oldType}, function(jsonobj) {
						if(!!jsonobj.error) {
							$.alert(jsonobj.error);
				...
				if($("#layerlist").height() > 325) {
					$("#layerlist").css({width:"180px"});
				}
				else {
					$("#layerlist").css({width:"197px"});
				}
			};
			populateLayers();
		...
		// 	}
			...
			var Actions = function() {
				...
				var tool_buttons = [
					...
					{sel:'#tool_save', fn: function() {saveSvg()}, evt: 'mouseup', key: ['S', true]},
