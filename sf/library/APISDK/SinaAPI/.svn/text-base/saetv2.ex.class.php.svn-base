...
define('SinaAPIKey',SF::app ()->params ['SinaAPIKey']);
define('SinaAPISec',SF::app ()->params ['SinaAPISec']);
...
class SaeTClientV2
{
	 ...
	 * 
	 * @access public
	 * @param mixed $target_id_name 目标用户的微博昵称
	 * @param mixed $source_id_name 源用户的微博昵称，可选，默认为当前的用户
	 * @return array
	 */
	function friendships_by_id_name( $target_id_name, $source_id_name )
	{
		$params = array();
		if (is_numeric($target_id_name))	$params['target_id'] = $target_id_name;
		else 	$params['target_screen_name'] = $target_id_name;

		if (is_numeric($source_id_name))	$params['source_id'] = $source_id_name;
		else 	$params['source_screen_name'] = $source_id_name;
