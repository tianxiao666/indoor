...
class ExtMinScript extends CApplicationComponent {
	...
	public function init() {
		...
		if (!extension_loaded('apc')) {
			$cachePath = SF::app() -> runtimePath . '/minScript/cache';
			if (!is_dir($cachePath)) {
	 ...
	 */
	private function _processGroupMap() {
		...
		foreach ($groupMap as $group => $items) {
			...
			foreach ($items as $index => $path) {
				...
				if (strpos($filename, '*') !== false) {
					SF::log('No asterisks in filename, skipping file ' . $path, 'warning', 'ext.minScript');
		...
		if (!empty($group)) {
			if (isset($groupMap[$group])) {
				...
				foreach ($groupMap[$group] as $path) {
					$filename = basename($path);
					SF::app() -> clientScript -> scriptMap[$filename] = $minScriptUrl;
				}
			}
		} else {
			//Groups
	...
	public function generateUrl($group) {
		...
		if (isset($groupMap[$group])) {
			...
			foreach ($groupMap[$group] as $path) {
				...
				if ($filemtime !== false) {
					...
					SF::log('Can\'t access ' . $path, 'error', 'ext.minScript');
					$noFilemtime += 1;
				}
			}
		...
		$minScriptUrl = SF::app() -> createUrl($this -> controllerID . '/serve', $params);
		return $minScriptUrl;
	}
