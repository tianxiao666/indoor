...
SF::import('system.library.image.Image_Driver');

/**
 * Manipulate images using standard methods such as resize, crop, rotate, etc.
 * This class must be re-initialized for every image you wish to manipulate.
 *
 * $Id: Image.php,v 1.6 2011/07/09 07:44:54 guzhiyong Exp $
 *
 * @package    Image
 * @author     Kohana Team
...
class Image {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(Image::$allowed_types[$image_info[2]])){
			...
			//throw new CException('image type not allowed'.$image_info[2]);
			SF::log('image type not allowed'.$image_info[2]);
			SF::log($image_info);
			return false;
		}
		...
		//require(SF_PATH.'/library/image/drivers/$driver');
		//SF::log(SF_PATH.'/library/image/drivers/'.$driver);
        // Load the driver
        SF::import("system.library.image.drivers.$driver");
		//return false;
	 ...
	 * @return object
	 */
	public function cut($width,$height,$master,$x=0,$y=0)
	{
		$this->resize($width, $height,$master);
		$x = $x ==-1 ?0:$x;
		$y = $y ==-1 ?0:$y;
		$this->crop($width, $height, $top = $x, $left = $y);
		return $this;
	}
    /**
