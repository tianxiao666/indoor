 ...
 * GD Image Driver.
 *
 * $Id: Image_GD_Driver.php,v 1.4 2011/06/07 09:56:02 huanggz Exp $
 *
 * @package    Image
 * @author     Kohana Team
...
class Image_GD_Driver extends Image_Driver {
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] == Image::EQUAL)
		{
			//改变后的图象的比例1:1
			$resize_ratio = ($properties['width'])/($properties['height']);
			SF::log("resize_ratio  ".$resize_ratio);
			//实际图象的比例　$this->img["lebar"]=width 
			$ratio = ($width )/($height);
			SF::log("ratio  ".$ratio);
			//target image's width/height bigger then source image's width/height
			//if source image's width is bigger than height, then expand source image by height
			if($ratio > $resize_ratio)
			{
				if($properties['height'] != $height)
				{
					$width=round($width*$properties['height']/$height);
					$height=$properties['height'];
				}
			}
			//if source image's height is bigger than width, then expand source image by width
			else if($ratio < $resize_ratio)
			{
				if($properties['width'] != $width)
				{
					$height *= $properties['width']/$width;
					$width = $properties['width'];
				}
			}
			$re_img = $this->imagecreatetransparent($width, $height);
			imagecopyresampled($re_img, $this->tmp_image, 0, 0, 0, 0, $width, $height, imagesx($this->tmp_image), imagesy($this->tmp_image));
			imagedestroy($this->tmp_image);
			$this->tmp_image = $re_img;
			// Set the "save" function
			switch (strtolower(substr(strrchr($this->image['file'], '.'), 1)))
			{
				case 'jpg':
				case 'jpeg':
					$save = 'imagejpeg';
...
//			$save($this->tmp_image, $this->image['file']);
			
			$src_x = round((imagesx($this->tmp_image)-$properties['width'])/2);
			$src_y = round((imagesy($this->tmp_image)-$properties['height'])/2);
			// Create the temporary image to copy to
			$img = $this->imagecreatetransparent($properties['width'], $properties['height']);

			// Execute the resize
			$status = imagecopy($img, $this->tmp_image,0, 0, $src_x, $src_y, imagesx($this->tmp_image), imagesy($this->tmp_image));
			if ($status)
			{
				// Swap the new image for the old one
				imagedestroy($this->tmp_image);
		...
		else 
		{
			...
			if ($properties['width'] > $width / 2 AND $properties['height'] > $height / 2)
			{
				...
				SF::log("pre_height  ".$properties['pre_height']);
				SF::log("pre_width  ".$properties['pre_width']);
				// Create the temporary image to copy to
		...
		function size_cute($properties)
		{
			// Get the current width and height
			$image_width = imagesx($this->tmp_image);
			$image_height = imagesy($this->tmp_image);
