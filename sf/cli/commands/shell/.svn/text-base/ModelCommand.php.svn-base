 ...
 * @version $Id: ModelCommand.php,v 1.2 2010/04/16 07:10:34 liujz Exp $
 */
...
//							)
	 ...
	 * @param array the database $tables
	 * @param string a regular expression that may be used to filter table names
	 */
	protected function generateClassNames($tables,$pattern=null)
	{
		$this->_tables=array();
...
//		$this->_schema=$db->schema;
		...
		{
			$this->generateClassNames($tables);
			if(($pos=strrpos($className,'.'))===false)
				$basePath=SF::getPathOfAlias('application.models');
			else
			{
				$basePath=SF::getPathOfAlias(substr($className,0,$pos));
				$className=substr($className,$pos+1);
			}
			if($className==='*') // generate all models
				$this->_classes=$this->_tables;	
			else
			{
		...
		else  // with regular expression
		{
			...
			if($pos>0)  // only regexp is given
				$basePath=SF::getPathOfAlias(rtrim(substr($className,0,$pos),'.'));
			else
				$basePath=SF::getPathOfAlias('application.models');
...
//			$this->generateRelations();
		...
		$templatePath=$this->templatePath===null?SF_PATH.'/cli/views/shell/model':$this->templatePath;
//		$fixturePath=$this->fixturePath===null?SF::getPathOfAlias('application.tests.fixtures'):$this->fixturePath;
//		$unitTestPath=$this->unitTestPath===null?SF::getPathOfAlias('application.tests.unit'):$this->unitTestPath;
...
//			echo "Warning: the table '$tableName' does not exist in the database.\n";
		...
		if(!is_file($source))  // fall back to default ones
			$source=SF_PATH.'/cli/views/shell/model/'.basename($source);
...
//	}
	...
	{
//		$tablePrefix=SF::app()->getDb()->tablePrefix;
