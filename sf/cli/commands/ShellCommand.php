 ...
 * @version $Id: ShellCommand.php,v 1.1 2010/03/16 08:05:37 liujz Exp $
 */
...
class ShellCommand extends CConsoleCommand
{
	...
	public function getHelp()
	{
		return <<<EOD
USAGE
  SFc shell [entry-script | config-file]
...
EOD;
	...
	public function run($args)
	{
		...
		SF::setApplication(null);
		SF::setPathOfAlias('application',null);
		...
		if(is_array($config))
		{
			...
			SF::createWebApplication($config);
		}

		restore_error_handler();
		restore_exception_handler();

		$SFVersion=SF::getVersion();
		echo <<<EOD
SF Interactive Tool v1.1 (based on SF v{$SFVersion})
...
EOD;
	...
	protected function runShell()
	{
		...
		$_runner_->addCommands(SF::getPathOfAlias('application.autorun'));
		if(($_path_=@getenv('SFC_SHELL_COMMAND_PATH'))!==false)
