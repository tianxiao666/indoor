...
class CLogger extends CComponent
{
	 ...
	 * set to 1 for write log imediate.
	 * @since 1.1.0
	 */
	public $autoFlush=1;
	/**
	...
	{
		// add array support, by liujz 2009-07-08
	 ...
	 * defined by constant SF_BEGIN_TIME.
	...
	{
		return microtime(true)-SF_BEGIN_TIME;
	}
		...
		foreach($this->_logs as $log)
		{
			...
			else if(!strncasecmp($message,'end:',4))
			{
				...
				if(($last=array_pop($stack))!==null && $last[0]===$token)
				{
					...
					throw new CException(SF::t('SF','CProfileLogRoute found a mismatching code block "{token}". Make sure the calls to SF::beginProfile() and SF::endProfile() be properly nested.',
						array('{token}'=>$token)));
			}
		}
	 ...
	 * in class CLogAppCompo, when it init ,it will attach an Event Handler to this class.
	 * CLogAppCompo will collect logs from this logger ,log route send log to file,email...
