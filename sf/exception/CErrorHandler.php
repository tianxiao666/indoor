 ...
 * (i.e. SF_DEBUG is defined as true). Detailed error information with source code
 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CErrorHandler.php,v 1.5 2011/07/28 03:21:41 duhw Exp $
 * @package system.base
 * @since 1.0
 */
class CErrorHandler extends CApplicationComponent
{
	 ...
	 * Inside the action, it can retrieve the error information by SF::app()->errorHandler->error.
	...
	protected function handleException($exception)
	{
		...
		if($app instanceof CWebApplication)
		{
			...
			//if($exception instanceof CHttpException || !SF_DEBUG)
	...
	/**
	 * Renders the view.
	 * BY HAKA::2012/12/06  Uncatch exception and error,please refer to CApplication::displayException
	...
	{
		//BY HAKA,MAKE IT SIMPLE,DO NOT JUMP TO ANOTHER CONTROLLER.
		if($view==='error' && $this->errorAction!==null){
			SF::log('------------------ Error,redirect to defined erroraction:'.$this->errorAction);
			//20121206,not to jump to another controller,should render an error page and return.
			SF::app()->runController($this->errorAction);
		} else
		{
			$viewFile = $this->getViewFile($view,$data['code']);
			if (is_file($viewFile))
				include($viewFile);
			else
			{
				...
				echo "<b>Message:".$data['message'].'</b><br>';
				echo "<b>File</b>:".$data['file'].'::<b>  Line</b>-->'.$data['line'].'<br>';
				echo "<br><b>trace</b>:<br>".$data['trace'].'<br>';
				print_r('<br><b>source</b>::<br>');
				print_r($data['source']);
			}
		}
	}
	...
	/**
	 * //BY HAKA,201206,should rewrite the function by using SMARTY TEMPL.
	...
	{
		$viewPath = SF::app()->getSystemViewPath();
		return $this->getViewFileInternal($viewPath, $view, $code);
	}
	...
	protected function getVersionInfo()
	{
		if(SF_DEBUG)
		{
			$version='<a href="http://www.SFframework.com/">SF Framework</a>/'.SF::getVersion();
