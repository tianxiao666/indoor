 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CCache.php,v 1.2 2010/03/31 08:34:17 liujz Exp $
...
abstract class CCache extends CApplicationComponent implements ICache, ArrayAccess
{
	...
	public function get($id)
	{
		if(($value=$this->getValue($this->generateUniqueKey($id)))!==false)
		{
			...
			{
				SF::trace('Serving "'.$id.'" from cache','system.caching.'.get_class($this));
				return $data[0];
			}
		}
		return false;
	...
	{
		SF::trace('Saving "'.$id.'" to cache','system.caching.'.get_class($this));
		//if($dependency!==null)
	...
	{
		SF::trace('Adding "'.$id.'" to cache','system.caching.'.get_class($this));
		//if($dependency!==null)
		//	$dependency->evaluateDependency();
		$data=array($value,$dependency);
	...
	{
		SF::trace('Deleting "'.$id.'" to cache','system.caching.'.get_class($this));
	...
	{
		SF::trace('Flushign cache','system.caching.'.get_class($this));
		throw new CException(SF::t('SF','{className} does not support flush() functionality.',
	...
	{
		throw new CException(SF::t('SF','{className} does not support get() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(SF::t('SF','{className} does not support set() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(SF::t('SF','{className} does not support add() functionality.',
			array('{className}'=>get_class($this))));
	}
	...
	{
		throw new CException(SF::t('SF','{className} does not support delete() functionality.',
			array('{className}'=>get_class($this))));
	}
