 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CMemCache.php,v 1.2 2010/03/31 08:34:17 liujz Exp $
...
class CMemCache extends CCache
{
	...
	public function init()
	{
		...
		if(!extension_loaded('memcache'))
			throw new CException(SF::t('SF','CMemCache requires PHP memcache extension to be loaded.'));
	...
	public function __construct($config)
	{
		if(is_array($config))
		{
			foreach($config as $key=>$value)
				$this->$key=$value;
			if($this->host===null)
				throw new CException(SF::t('SF','CMemCache server configuration must have "host" value.'));
		}
		else
			throw new CException(SF::t('SF','CMemCache server configuration must be an array.'));
	}
}