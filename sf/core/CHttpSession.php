 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CHttpSession.php,v 1.6 2009/12/30 01:52:37 liujz Exp $
 * @package system.web
 * @since 1.0
 */
class CHttpSession extends CApplicationComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	/**
	 * @var string session name,the name of user visit session
	 */
	public $sessionName;
	
	/**
	 * @var string session domain, the domain of the session
	 */
	...
	public function open()
	{
		if(session_id()==='')
		{
			...
			session_set_cookie_params(0, '/', $this->sessionDomain);
	...
	{
		if(is_dir($value) || substr($value, 0, 2) == '2;')
			session_save_path($value);
		else
			throw new CException(SF::t('SF','CHttpSession.savePath "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function setCookieMode($value)
	{
		...
		else if($value==='only')
		{
			...
			throw new CException(SF::t('SF','CHttpSession.cookieMode can only be "none", "allow" or "only".'));
	}
	...
	public function setGCProbability($value)
	{
		...
		if($value>=0 && $value<=100)
		{
			...
			throw new CException(SF::t('SF','CHttpSession.gcProbability "{value}" is invalid. It must be an integer between 0 and 100.',
				array('{value}'=>$value)));
	}
