...
/**
 * CDataModel is the base class providing the common features needed by data model objects.
 *
 * CDataModel defines the basic framework for data models that need to be validated.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CDataModel.php,v 1.1 2009/05/13 06:31:11 haka Exp $
 * @package system.base
 * @since 1.0
 */
abstract class CDataModel extends CComponent implements IteratorAggregate, ArrayAccess
{
	...
	public function createValidators()
	{
		...
		foreach($this->rules() as $rule)
		{
			if(isset($rule[0],$rule[1]))  // attributes, validator name
				...
				throw new CException(SF::t('SF','{class} has an invalid validation rule. The rule must specify attributes to be validated and the validator name.',
