 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CWebApplication.php,v 1.9 2011/07/25 09:44:49 mary Exp $
 * @package system.web
 * @since 1.0
 */
class CWebApplication extends CApplication
{
	...
	{
		//$this->getAntiInjectionSQL();
		//SF::log($_REQUEST);
		//SF::log($_POST);
		$this->getAntiXssAttack();
		...
		// check the route format is value or not, liujz 2010-4-19
		if (!$this->checkRouteFormat($route))
		{
			$oErrFile = SF::app()->getErrorFileLog();
			$oErrFile->processErrorLogs("route = {$route} is not valid, application end");
			SF::app()->end();
		}
		// check end
	...
	/**
	 * \brief check the route format
	 */
	private function checkRouteFormat($route)
	{
		$opList = explode('/', $route);
		if (count($opList) > 2)
			return FALSE;
	...
	{
		return $this->getComponent('sms');
	}	
	
	public function getAntiInjectionSQL()
	{
		return $this->getComponent('AntiInjectionSQL');
	}	
	
	public function getAntiXssAttack()
	{
		return $this->getComponent('AntiXssAttack');
	}	
	...
	public function runController($route)
	{
		if(($ca=$this->createController($route))!==null)
		{
			...
			throw new CHttpException(404,SF::t('SF','Unable to resolve the request "{route}".',
	...
	public function createController($route,$owner=null)
	{
		...
		while(($pos=strpos($route,'/'))!==false)
		{
			...
			if(!isset($basePath))  // first segment
			{
				...
				{
					return array(
						SF::createComponent($owner->controllerMap[$id],$id,$owner===$this?null:$owner),
	...
	public function setSystemViewPath($path)
	{
		if(($this->_systemViewPath=realpath($path))===false || !is_dir($this->_systemViewPath))
			throw new CException(SF::t('SF','The system view path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
