 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CHttpRequest.php,v 1.2 2011/04/19 10:39:53 mary Exp $
 * @package system.web
 * @since 1.0
 */
class CHttpRequest extends CApplicationComponent
{
	...
	/**
	 * @var string the name of the token used to prevent CSRF. Defaults to 'SF_CSRF_TOKEN'.
	 * This property is effectively only when {@link enableCsrfValidation} is true.
	 */
	public $csrfTokenName='SF_CSRF_TOKEN';
	/**
	...
	protected function normalizeRequest()
	{
		...
		if($this->enableCsrfValidation)
			SF::app()->attachEventHandler('onBeginRequest',array($this,'validateCsrfToken'));
	}
	 ...
	 * @return string schema and hostname part (with port number if needed) of the request URL (e.g. http://www.SFframework.com)
	 * @see setHostInfo
	 */
	...
	public function getScriptUrl()
	{
		if($this->_scriptUrl===null)
		{
			...
			if(basename($_SERVER['SCRIPT_NAME'])===$scriptName)
				...
				throw new CException(SF::t('SF','CHttpRequest is unable to determine the entry script URL.'));
		}
	...
	public function getPathInfo()
	{
		if($this->_pathInfo===null)
		{
			...
			if(strpos($requestUri,$scriptUrl)===0)
				...
				throw new CException(SF::t('SF','CHttpRequest is unable to determine the path info of the request.'));
	...
	public function getRequestUri()
	{
		if($this->_requestUri===null)
		{
			...
			else if(isset($_SERVER['ORIG_PATH_INFO']))  // IIS 5.0 CGI
			{
				...
				throw new CException(SF::t('SF','CHttpRequest is unable to determine the request URI.'));
		}
	...
	public function validateCsrfToken($event)
	{
		if($this->getIsPostRequest())
		{
			...
			if($cookies->contains($this->csrfTokenName) && isset($_POST[$this->csrfTokenName]))
			{
				...
				$valid=false;
			if(!$valid)
				throw new CHttpException(400,SF::t('SF','The CSRF token could not be verified.'));
		}
	}
}
	/*@author mary
...
class CCookieCollection extends CMap
{
	...
	public function add($name,$cookie)
	{
		if($cookie instanceof CHttpCookie)
		{
			...
			throw new CException(SF::t('SF','CHttpCookieCollection can only hold CHttpCookie objects.'));
	}
	...
	protected function addCookie($cookie)
	{
		$value=$cookie->value;
		if($this->_request->enableCookieValidation)
			$value=SF::app()->getSecurityManager()->hashData($value);
