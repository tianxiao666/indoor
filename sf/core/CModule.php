 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CModule.php,v 1.1 2009/05/13 06:31:11 haka Exp $
 * @package system.base
 * @since 1.0.4
 */
abstract class CModule extends CComponent
{
	...
	public function __construct($id,$parent,$config=null)
	{
		...
		SF::setPathOfAlias($id,$this->getBasePath());
	...
	public function setBasePath($path)
	{
		if(($this->_basePath=realpath($path))===false || !is_dir($this->_basePath))
			throw new CException(SF::t('SF','Base path "{path}" is not a valid directory.',
				array('{path}'=>$path)));
	}
	...
	public function setModulePath($value)
	{
		if(($this->_modulePath=realpath($value))===false || !is_dir($this->_modulePath))
			throw new CException(SF::t('SF','The module path "{path}" is not a valid directory.',
				array('{path}'=>$value)));
	}
	...
	public function getModule($id)
	{
		...
		else if(isset($this->_moduleConfig[$id]))
		{
			...
			{
				SF::trace("Loading \"$id\" module",'system.base.CModule');
				$class=$config['class'];
				unset($config['class'], $config['enabled']);
				if($this===SF::app())
					$module=SF::createComponent($class,$id,null,$config);
				else
					$module=SF::createComponent($class,$this->getId().'/'.$id,$this,$config);
	...
	public function setModules($modules)
	{
		foreach($modules as $id=>$module)
		{
			...
			{
				SF::setPathOfAlias($id,$this->getModulePath().DIRECTORY_SEPARATOR.$id);
	...
	public function getComponent($id)
	{
		...
		else if(isset($this->_componentConfig[$id]))
		{
			...
			if(!isset($config['enabled']) || $config['enabled'])
			{
				SF::trace("Loading \"$id\" application component",'system.web.CModule');
				unset($config['enabled']);
				$component=SF::createComponent($config);
				$component->init();
	 ...
	 *         'class'=>'CDbCache',
	 *         'connectionID'=>'db',
	 *         'enabled'=>!SF_DEBUG,  // enable caching in non-debug mode
	 *     ),
	 * )
	 * </pre>
	 *
