 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CFilterChain.php,v 1.1 2011/10/22 06:22:38 huanggz Exp $
...
class CFilterChain extends CList
{
        ...
        public static function create($controller,$action,$filters)
        {
                ...
                foreach($filters as $filter)
                {
                        ...
                        {
                                if(!isset($filter[0]))
                                        throw new CException(SF::t('yii','The first element in a filter configuration must be the filter class.'));
                                ...
                                $filter=SF::createComponent($filter);
                        }
        ...
        public function insertAt($index,$item)
        {
                if($item instanceof IFilter)
                        ...
                        throw new CException(SF::t('yii','CFilterChain can only take objects implementing the IFilter interface.'));
        }
        ...
        public function run()
        {
                if($this->offsetExists($this->filterIndex))
                {
                        ...
                        SF::trace('Running filter '.($filter instanceof CInlineFilter ? get_class($this->controller).'.filter'.$filter->name.'()':get_class($filter).'.filter()'),'system.web.filters.CFilterChain');
                        $filter->filter($this);
                }
                else
