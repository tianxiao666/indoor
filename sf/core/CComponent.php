 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CComponent.php,v 1.1 2009/05/13 06:31:11 haka Exp $
 * @package system.base
 * @since 1.0
 */
class CComponent
{
	...
	public function __get($name)
	{
		...
		else if(strncasecmp($name,'on',2)===0 && method_exists($this,$name))
		{
			...
			throw new CException(SF::t('SF','Property "{class}.{property}" is not defined.',
	...
	public function __set($name,$value)
	{
		...
		else if(strncasecmp($name,'on',2)===0 && method_exists($this,$name))
		{
			...
			throw new CException(SF::t('SF','Property "{class}.{property}" is read only.',
				array('{class}'=>get_class($this), '{property}'=>$name)));
		else
			throw new CException(SF::t('SF','Property "{class}.{property}" is not defined.',
				array('{class}'=>get_class($this), '{property}'=>$name)));
	...
	public function __call($name,$parameters)
	{
		...
		throw new CException(SF::t('SF','{class} does not have a method named "{name}".',
	...
	public function attachBehavior($name,$behavior)
	{
		if(!($behavior instanceof IBehavior))
			$behavior=SF::createComponent($behavior);
	...
	public function getEventHandlers($name)
	{
		if($this->hasEvent($name))
		{
			...
			throw new CException(SF::t('SF','Event "{class}.{event}" is not defined.',
	...
	public function raiseEvent($name,$event)
	{
			...
			foreach($this->_e[$name] as $handler)
			{
				...
				else if(is_callable($handler,true))
				{
					...
					if(is_string($object))	// static method call
						...
						throw new CException(SF::t('SF','Event "{class}.{event}" is attached with an invalid handler "{handler}".',
		...
		else if(SF_DEBUG && !$this->hasEvent($name))
			throw new CException(SF::t('SF','Event "{class}.{event}" is not defined.',
				array('{class}'=>get_class($this), '{event}'=>$name)));
	}
}
