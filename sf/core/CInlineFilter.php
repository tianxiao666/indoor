 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CInlineFilter.php,v 1.1 2011/10/22 06:22:38 huanggz Exp $
...
class CInlineFilter extends CFilter
{
        ...
        public static function create($controller,$filterName)
        {
                if(method_exists($controller,'filter'.$filterName))
                {
                        ...
                        throw new CException(SF::t('yii','Filter "{filter}" is invalid. Controller "{class}" does not have the filter method "filter{filter}".',
