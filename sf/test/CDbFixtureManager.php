 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CDbFixtureManager.php,v 1.1 2010/03/25 06:20:13 liujz Exp $
 * @package system.test
 * @since 1.1
 */
class CDbFixtureManager extends CApplicationComponent
{
	...
	public function init()
	{
		...
		if($this->basePath===null)
			$this->basePath=SF::getPathOfAlias('webroot.tests.fixtures');
		$this->prepare();
	}
	...
	public function getDbConnection()
	{
		if($this->_db===null)
		{
			$this->_db=SF::app()->getComponent($this->connectionID)->getDBConnection();
			if(!$this->_db instanceof ADOConnection)
				throw new CException(SF::t('SF','CDbTestFixture.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.',
...
//		$builder=$schema->getCommandBuilder();
	 ...
	 * @return array the names of the model that have fixture data
	 */
	...
	public function load($fixtures)
	{
		...
		foreach($fixtures as $fixtureName=>$tableName)
		{
			...
			{
				$modelClass=SF::import($tableName,true);
	...
	/**
	 * \brief check the table whether exist
	 * @param unknown_type $table
	 */
	private function getTable($table=false)
	{
		if (!$this->_db)
			$this->getDbConnection();
		
		$tables = $this->_db->MetaTables('TABLE');
