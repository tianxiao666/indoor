 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CQueue.php,v 1.2 2010/03/31 08:34:16 liujz Exp $
...
class CQueue extends CComponent implements IteratorAggregate,Countable
{
	...
	public function copyFrom($data)
	{
		if(is_array($data) || ($data instanceof Traversable))
		{
			...
			throw new CException(SF::t('SF','Queue data must be an array or an object implementing Traversable.'));
	}
	...
	{
		if($this->_c===0)
			throw new CException(SF::t('SF','The queue is empty.'));
		else
			return $this->_d[0];
	}
