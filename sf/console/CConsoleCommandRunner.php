 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CConsoleCommandRunner.php,v 1.1 2010/03/12 09:59:34 liujz Exp $
...
class CConsoleCommandRunner extends CComponent
{
	 ...
	 * {@link SFBase::getPathOfAlias class path alias} of the command.
	 * If the latter, the array must contain a 'class' element which specifies
	 * the command's class name or {@link SFBase::getPathOfAlias class path alias}.
		...
		if(isset($this->commands[$name]))
		{
			if(is_string($this->commands[$name]))  // class file path or alias
			{
				if(strpos($this->commands[$name],'/')!==false || strpos($this->commands[$name],'\\')!==false)
				{
					...
					$className=SF::import($this->commands[$name]);
				return new $className($name,$this);
			}
			else // an array configuration
				return SF::createComponent($this->commands[$name],$name,$this);
		}
		else if($name==='help')
