 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CCaptchaValidator.php,v 1.2 2010/03/31 08:32:39 liujz Exp $
...
class CCaptchaValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if(($captcha=SF::app()->getController()->createAction($this->captchaAction))===null)
		{
			if(strpos($this->captchaAction,'/')!==false) // contains controller or module
			{
				if(($ca=SF::app()->createController($this->captchaAction))!==null)
				{
			...
			if($captcha===null)
				throw new CException(SF::t('SF','CCaptchaValidator.action "{id}" is invalid. Unable to find such an action in the current controller.',
						array('{id}'=>$this->captchaAction)));
		}
		if(!$captcha->validate($value,$this->caseSensitive))
		{
			$message=$this->message!==null?$this->message:SF::t('SF','The verification code is incorrect.');
