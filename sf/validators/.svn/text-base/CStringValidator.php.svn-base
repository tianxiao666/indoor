 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CStringValidator.php,v 1.2 2010/03/31 08:32:39 liujz Exp $
...
class CStringValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		{
			$message=$this->tooShort!==null?$this->tooShort:SF::t('SF','{attribute} is too short (minimum is {min} characters).');
			$this->addError($object,$attribute,$message,array('{min}'=>$this->min));
		}
		if($this->max!==null && $length>$this->max)
		{
			$message=$this->tooLong!==null?$this->tooLong:SF::t('SF','{attribute} is too long (maximum is {max} characters).');
			$this->addError($object,$attribute,$message,array('{max}'=>$this->max));
		}
		if($this->is!==null && $length!==$this->is)
		{
			$message=$this->message!==null?$this->message:SF::t('SF','{attribute} is of the wrong length (should be {length} characters).');
