 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CCompareValidator.php,v 1.2 2010/03/31 08:32:39 liujz Exp $
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		switch($this->operator)
		{
			...
			case '==':
				...
				{
					$message=$this->message!==null?$this->message:SF::t('SF','{attribute} must be repeated exactly.');
			...
			case '!=':
				if(($this->strict && $value===$compareValue) || (!$this->strict && $value==$compareValue))
				{
					$message=$this->message!==null?$this->message:SF::t('SF','{attribute} must not be equal to "{compareValue}".');
			...
			case '>':
				...
				{
					$message=$this->message!==null?$this->message:SF::t('SF','{attribute} must be greater than "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			case '>=':
				if($value<$compareValue)
				{
					$message=$this->message!==null?$this->message:SF::t('SF','{attribute} must be greater than or equal to "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			case '<':
				if($value>=$compareValue)
				{
					$message=$this->message!==null?$this->message:SF::t('SF','{attribute} must be less than "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			case '<=':
				if($value>$compareValue)
				{
					$message=$this->message!==null?$this->message:SF::t('SF','{attribute} must be less than or equal to "{compareValue}".');
					$this->addError($object,$attribute,$message,array('{compareAttribute}'=>$compareTo,'{compareValue}'=>$compareValue));
				}
				break;
			default:
				throw new CException(SF::t('SF','Invalid operator "{operator}".',array('{operator}'=>$this->operator)));
		}
	}
}
