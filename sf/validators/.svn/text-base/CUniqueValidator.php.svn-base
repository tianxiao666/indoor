 ...
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: CUniqueValidator.php,v 1.2 2010/03/31 08:32:39 liujz Exp $
...
class CUniqueValidator extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$className=$this->className===null?get_class($object):SF::import($this->className);
		$attributeName=$this->attributeName===null?$attribute:$this->attributeName;
		$finder=CActiveRecord::model($className);
		$table=$finder->getTableSchema();
		if(($column=$table->getColumn($attributeName))===null)
			throw new CException(SF::t('SF','Column "{column} does not exist in table "{table}".',
		...
		if($exists)
		{
			$message=$this->message!==null?$this->message:SF::t('SF','{attribute} "{value}" has already been taken.');
